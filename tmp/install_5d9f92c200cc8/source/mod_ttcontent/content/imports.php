<?php
				...
				$db->setQuery($query);
				$db->execute();
				/*$db->setQuery("SELECT menu.id FROM #__menu_types menu WHERE menu.title='theme--2001-menu' AND menu.menutype='custommenu'");
				$mid = $db->loadResult();
				if(empty($mid))
				{*/
				// create TT-menu
			...
			private function upload_media($type)
				{
					jimport('joomla.filesystem.folder');
					$path = JPATH_SITE . "/"."images"."/theme--2001/";
					$mode = 0755;
				...
				while ($tt_read_image = readdir($tt_content_images)) 
				{
					...
					{
					$var2 = implode(DS,$rootFolder). "/"."images"."/theme--2001/" . $tt_read_image;
				...
				{
				$url1 = JURI::root().'images/theme--2001';
